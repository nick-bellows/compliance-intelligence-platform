...
| Public CI | `a530056`: container smoke test, pip-audit, and gitleaks green; the drift test failed because the committed report hashed a CRLF working copy of the labeled set and recorded a Windows path (every score identical). The follow-up commit hashes LF-normalized bytes and records a POSIX path, making the report platform-independent. `62a8ff7`: all four jobs green (lint/types/tests/drift checks, container build and smoke test, pip-audit, gitleaks) |
| Cold clone from GitHub | `a530056`: fresh Python 3.13 venv, ruff and mypy clean, 71 passed / 2 skipped (NLP and retrieval extras not installed) plus the same single drift failure. `62a8ff7`: 72 passed / 2 skipped, ruff and mypy clean, manifest valid, walkthrough not stale, tracked tree clean after the suite, CLI entry point runs |
