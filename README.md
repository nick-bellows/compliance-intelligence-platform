...
The health endpoint is available immediately. The screening endpoint returns `503` until a verified dataset snapshot is loaded; this prevents an empty dataset from producing false “clear” results.
...
The API port is bound to `127.0.0.1`. The default container still has no authoritative dataset loader wired into application startup, so screening remains unavailable until M1 implements and verifies that boundary.
